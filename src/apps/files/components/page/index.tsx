...
export function PageContent({ className = '', ...props }: React.ComponentPropsWithoutRef<'div'>) {
  return <div className={`mt-1.5 min-h-0 flex-1 overflow-hidden ${className}`} {...props} />;
}
...
export function PageHeader({
  ...
  return (
    <div className="mx-auto w-full px-5" {...props}>
      <div className={`flex w-full items-center justify-between pb-6 ${className}`}>{children}</div>
    </div>
  );
}
...
export function PageHeaderActions({
  ...
  return <div className={`flex shrink-0 gap-4 ${className}`} {...props} />;
}
