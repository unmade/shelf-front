...
interface ItemData {
  ids: string[];
  entities: Record<string, IMediaItem>;
  columnCount: number;
}
...
const GridViewCellRenderer = memo(function GridViewCellRenderer({
  ...
  const mediaItemId = data.ids[itemIndex];
  const mediaItem = data.entities[mediaItemId];
  ...
  return (
    <div style={style}>
      <GridViewItem mediaItem={mediaItem} />
    </div>
  );
});
...
interface GridViewProps {
  data: EntityState<IMediaItem, string>;
}

export function GridView({ data }: GridViewProps) {
  const { t } = useTranslation('photos');

  const { loadMore } = useMediaItemsBrowserData();
  ...
  const itemData = useMemo<ItemData>(
    () => ({
      ids,
      entities: data.entities as Record<string, IMediaItem>,
      columnCount,
    }),
  ...
  return (
    <div
      ...
      role="grid"
      aria-label={t('browser.grid.ariaLabel', { defaultValue: 'Photos grid' })}
    >
      <VGrid
