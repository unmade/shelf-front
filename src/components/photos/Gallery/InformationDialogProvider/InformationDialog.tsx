...
export default function InformationDialog({ mediaItem, open, onClose }: Props) {
        ...
        <DialogBody>
          ...
          <div className="pt-4">
            <p className="mb-1 px-2 text-left text-gray-800 dark:text-zinc-100">
              <TimeAgo value={mediaItem?.modifiedAt ?? 0} format="LLLL" />
            </p>
            {mediaItem && <MediaItemExif mediaItemId={mediaItem.id} />}
            <div className="mt-6">{mediaItem && <Categories mediaItemId={mediaItem.id} />}</div>
          </div>
        </DialogBody>
        <DialogFooter>
          <DialogClose asChild>
